...
library GovernorStateLogicV5 {
  ...
  function _state(
    ...
    uint256 currentTimepoint = GovernorClockLogicV5.clock(self);
    uint256 deadline = GovernorProposalLogicV5._proposalDeadline(self, proposalId);

    if (!GovernorDepositLogicV5.proposalDepositReached(self, proposalId)) {
    ...
    if (deadline >= currentTimepoint) {
      ...
      !GovernorQuorumLogicV5.quorumReached(self, proposalId) || !GovernorVotesLogicV5.voteSucceeded(self, proposalId)
    ) {
      return GovernorTypes.ProposalState.Defeated;
    } else if (GovernorProposalLogicV5.proposalEta(self, proposalId) == 0) {
