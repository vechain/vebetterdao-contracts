...
import { GovernorConfiguratorV5 } from "./GovernorConfiguratorV5.sol";
import { GovernorProposalLogicV5 } from "./GovernorProposalLogicV5.sol";
import { GovernorClockLogicV5 } from "./GovernorClockLogicV5.sol";
import { Math } from "@openzeppelin/contracts/utils/math/Math.sol";
import { Checkpoints } from "@openzeppelin/contracts/utils/structs/Checkpoints.sol";
import { SafeCast } from "@openzeppelin/contracts/utils/math/SafeCast.sol";

/// @title GovernorVotesLogicV5
...
library GovernorVotesLogicV5 {
  ...
  function castVote(
    ...
    GovernorStateLogicV5.validateStateBitmap(
      self,
      proposalId,
      GovernorStateLogicV5.encodeStateBitmap(GovernorTypes.ProposalState.Active)
    );

    uint256 proposalSnapshot = GovernorProposalLogicV5._proposalSnapshot(self, proposalId);
  ...
  function _checkVotingThreshold(GovernorStorageTypes.GovernorStorage storage self, uint256 weight) private view {
    uint256 threshold = GovernorConfiguratorV5.getVotingThreshold(self);
    if (weight < threshold) {
  ...
  function toggleQuadraticVoting(GovernorStorageTypes.GovernorStorage storage self) external {
    bool isQuadraticDisabled = self.quadraticVotingDisabled.upperLookupRecent(GovernorClockLogicV5.clock(self)) == 1; // 0: enabled, 1: disabled
    ...
    self.quadraticVotingDisabled.push(GovernorClockLogicV5.clock(self), newStatus);
