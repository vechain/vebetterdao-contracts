...
import { GovernorStateLogicV5 } from "./GovernorStateLogicV5.sol";
import { GovernorClockLogicV5 } from "./GovernorClockLogicV5.sol";
import { GovernorDepositLogicV5 } from "./GovernorDepositLogicV5.sol";
import { GovernorGovernanceLogicV5 } from "./GovernorGovernanceLogicV5.sol";
import { GovernorFunctionRestrictionsLogicV5 } from "./GovernorFunctionRestrictionsLogicV5.sol";
import { SafeCast } from "@openzeppelin/contracts/utils/math/SafeCast.sol";
import { Address } from "@openzeppelin/contracts/utils/Address.sol";
import { DoubleEndedQueue } from "@openzeppelin/contracts/utils/structs/DoubleEndedQueue.sol";

/// @title GovernorProposalLogicV5
...
library GovernorProposalLogicV5 {
  ...
  function queue(
    ...
    GovernorStateLogicV5.validateStateBitmap(
      self,
      proposalId,
      GovernorStateLogicV5.encodeStateBitmap(GovernorTypes.ProposalState.Succeeded)
    );
  ...
  function execute(
    ...
    GovernorStateLogicV5.validateStateBitmap(
      ...
      proposalId,
      GovernorStateLogicV5.encodeStateBitmap(GovernorTypes.ProposalState.Succeeded) |
        GovernorStateLogicV5.encodeStateBitmap(GovernorTypes.ProposalState.Queued)
    );
    ...
    if (GovernorGovernanceLogicV5.executor(self) != contractAddress) {
    ...
    if (GovernorGovernanceLogicV5.executor(self) != contractAddress && !self.governanceCall.empty()) {
  ...
  function cancel(
    ...
    GovernorStateLogicV5.validateStateBitmap(
      self,
      proposalId,
      GovernorStateLogicV5.ALL_PROPOSAL_STATES_BITMAP ^
        GovernorStateLogicV5.encodeStateBitmap(GovernorTypes.ProposalState.Canceled) ^
        GovernorStateLogicV5.encodeStateBitmap(GovernorTypes.ProposalState.Executed)
    );

    if (account == proposalProposer(self, proposalId)) {
      require(
        GovernorStateLogicV5._state(self, proposalId) == GovernorTypes.ProposalState.Pending,
  ...
  function _propose(
    ...
    uint256 depositThresholdAmount = GovernorDepositLogicV5._depositThreshold(self);
    ...
    if (depositAmount > 0) {
      GovernorDepositLogicV5.depositFunds(self, depositAmount, proposer, proposalId);
    }
  ...
  function validateProposeParams(
    ...
    if (self.proposals[proposalId].roundIdVoteStart != 0) {
      ...
      revert GovernorUnexpectedProposalState(proposalId, GovernorStateLogicV5._state(self, proposalId), bytes32(0));
    }

    GovernorFunctionRestrictionsLogicV5.checkFunctionsRestriction(self, targets, calldatas);
  }
  ...
  function _executeOperations(
    ...
    self.timelock.executeBatch{ value: msg.value }(
      ...
      calldatas,
      0,
      GovernorGovernanceLogicV5.timelockSalt(descriptionHash, contractAddress)
    );
    // cleanup for refund
    delete self.timelockIds[proposalId];
  }
  ...
  function _queueOperations(
    ...
    bytes32 salt = GovernorGovernanceLogicV5.timelockSalt(descriptionHash, contractAddress);
  ...
  function _canProposalStartInNextRound(
    ...
    uint48 currentBlock = GovernorClockLogicV5.clock(self);
  ...
  function _proposalSnapshot(
    ...
    if (currentRoundDeadline <= GovernorClockLogicV5.clock(self)) {
      currentRoundDeadline = GovernorClockLogicV5.clock(self);
    }
