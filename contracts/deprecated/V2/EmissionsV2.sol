...
import "../V4/interfaces/IXAllocationVotingGovernorV4.sol";
...
contract EmissionsV2 is AccessControlUpgradeable, ReentrancyGuardUpgradeable, UUPSUpgradeable {
  ...
  struct EmissionsStorage {
    ...
    IXAllocationVotingGovernorV4 xAllocationsGovernor; // XAllocationVotingGovernor contract
  ...
  function xAllocationsGovernor() public view returns (IXAllocationVotingGovernorV4) {
  ...
  function setCycleDuration(uint256 _cycleDuration) public virtual onlyRole(DECAY_SETTINGS_MANAGER_ROLE) {
    ...
    EmissionsStorage storage $ = _getEmissionsStorage();
    require(
      IXAllocationVotingGovernorV4($.xAllocationsGovernor).votingPeriod() < _cycleDuration,
  ...
  function setXAllocationsGovernorAddress(
    ...
    require(
      IXAllocationVotingGovernorV4(_xAllocationsGovernor).votingPeriod() < cycleDuration(),
      "Emissions: Voting period must be less than cycle duration"
    );

    EmissionsStorage storage $ = _getEmissionsStorage();
    emit XAllocationsGovernorAddressUpdated(_xAllocationsGovernor, address($.xAllocationsGovernor));
    $.xAllocationsGovernor = IXAllocationVotingGovernorV4(_xAllocationsGovernor);
  }
}
