...
import { INodeManagementV2 } from "./INodeManagement/INodeManagementV2.sol";
 ...
 */
contract NodeManagementV2 is INodeManagementV2, AccessControlUpgradeable, UUPSUpgradeable {
    ...
    function setVechainNodesContract(
        ...
        require(
            vechainNodesContract != address(0),
            "NodeManagement: vechainNodesContract cannot be the zero address"
        );
    ...
    function isNodeHolder(address user) public view returns (bool) {
        ...
        if ($.vechainNodesContract.ownerToId(user) != 0) {
            return true;
        }
    ...
    function getUserNodes(address user) public view returns (NodeInfo[] memory) {
        ...
        for (uint256 i = 0; i < nodeIds.length; i++) {
            ...
            nodesInfo[i] = NodeInfo({
                ...
                isXNodeDelegator: currentNodeId == ownedNodeId && isCurrentNodeDelegated,
                isXNodeDelegatee: currentNodeId != ownedNodeId,
    ...
    function getNodeLevel(
        uint256 nodeId
    ) public view returns (VechainNodesDataTypes.NodeStrengthLevel) {
        NodeManagementStorage storage $ = _getNodeManagementStorage();
    ...
    function getUsersNodeLevels(
        address user
    ) public view returns (VechainNodesDataTypes.NodeStrengthLevel[] memory) {
        // Retrieve the node IDs managed by the specified user
        uint256[] memory nodeIds = getNodeIds(user);

        // Initialize an array to hold the node levels
        VechainNodesDataTypes.NodeStrengthLevel[]
            memory nodeLevels = new VechainNodesDataTypes.NodeStrengthLevel[](nodeIds.length);
