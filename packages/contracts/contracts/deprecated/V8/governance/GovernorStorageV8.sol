...
import { GovernorStorageTypesV8 } from "./libraries/GovernorStorageTypesV8.sol";
import { GovernorTypesV8 } from "./libraries/GovernorTypesV8.sol";
import "@openzeppelin/contracts-upgradeable/proxy/utils/Initializable.sol";
import { IVeBetterPassport } from "../../../interfaces/IVeBetterPassport.sol";
import { Checkpoints } from "@openzeppelin/contracts/utils/structs/Checkpoints.sol";
import "@openzeppelin/contracts/utils/math/SafeCast.sol";
import { GovernorClockLogicV8 } from "./libraries/GovernorClockLogicV8.sol";
...
contract GovernorStorageV8 is Initializable {
  ...
  function getGovernorStorage() internal pure returns (GovernorStorageTypesV8.GovernorStorage storage $) {
    assembly {
  ...
  function __GovernorStorage_init(
    GovernorTypesV8.InitializationData memory initializationData,
  ...
  function __GovernorStorage_init_unchained(
    ...
    GovernorStorageTypesV8.GovernorStorage storage governorStorage = getGovernorStorage();
  ...
  function __GovernorStorage_init_v7(
    GovernorTypesV8.InitializationDataV7 memory initializationDataV7
  ) internal onlyInitializing {
    GovernorStorageTypesV8.GovernorStorage storage governorStorage = getGovernorStorage();

    // Set deposit threshold for Standard proposal type
    governorStorage.proposalTypeDepositThresholdPercentage[GovernorTypesV8.ProposalType.Standard] = governorStorage
      .depositThresholdPercentage_DEPRECATED;

    // Set deposit threshold for Grant proposal type
    governorStorage.proposalTypeDepositThresholdPercentage[GovernorTypesV8.ProposalType.Grant] = initializationDataV7
      .grantDepositThreshold;

    // Set voting threshold for Standard proposal type
    governorStorage.proposalTypeVotingThreshold[GovernorTypesV8.ProposalType.Standard] = governorStorage
      .votingThreshold_DEPRECATED;

    // Set voting threshold for Grant proposal type
    governorStorage.proposalTypeVotingThreshold[GovernorTypesV8.ProposalType.Grant] = initializationDataV7
    ...
    governorStorage.proposalTypeQuorum[GovernorTypesV8.ProposalType.Grant].push(
      GovernorClockLogicV8.clock(governorStorage),
      SafeCast.toUint208(initializationDataV7.grantQuorum)
    );

    // Set deposit threshold cap for Standard proposal type
    governorStorage.proposalTypeDepositThresholdCap[GovernorTypesV8.ProposalType.Standard] = initializationDataV7
      .standardDepositThresholdCap;

    // Set deposit threshold cap for Grant proposal type
    governorStorage.proposalTypeDepositThresholdCap[GovernorTypesV8.ProposalType.Grant] = initializationDataV7
      .grantDepositThresholdCap;

    // Set GM weight for Standard proposal type
    governorStorage.requiredGMLevelByProposalType[GovernorTypesV8.ProposalType.Standard] = initializationDataV7.standardGMWeight;

    // Set GM weight for Grant proposal type
    governorStorage.requiredGMLevelByProposalType[GovernorTypesV8.ProposalType.Grant] = initializationDataV7.grantGMWeight;
