// SPDX-License-Identifier: MIT

//                                      #######
//                                 ################
//                               ####################
//                             ###########   #########
//                            #########      #########
//          #######          #########       #########
//          #########       #########      ##########
//           ##########     ########     ####################
//            ##########   #########  #########################
//              ################### ############################
//               #################  ##########          ########
//                 ##############      ###              ########
//                  ############                       #########
//                    ##########                     ##########
//                     ########                    ###########
//                       ###                    ############
//                                          ##############
//                                    #################
//                                   ##############
//                                   #########

pragma solidity 0.8.20;

import { GovernorTypesV8 } from "./GovernorTypesV8.sol";
import { GovernorStorageTypesV8 } from "./GovernorStorageTypesV8.sol";
import { GovernorProposalLogicV8 } from "./GovernorProposalLogicV8.sol";
import { GovernorVotesLogicV8 } from "./GovernorVotesLogicV8.sol";
import { GovernorQuorumLogicV8 } from "./GovernorQuorumLogicV8.sol";
import { GovernorClockLogicV8 } from "./GovernorClockLogicV8.sol";
import { GovernorDepositLogicV8 } from "./GovernorDepositLogicV8.sol";

/// @title GovernorStateLogic
/// @notice Library for Governor state logic, managing the state transitions and validations of governance proposals.
library GovernorStateLogicV8 {
  /// @notice Bitmap representing all possible proposal states.
  bytes32 internal constant ALL_PROPOSAL_STATES_BITMAP =
    bytes32((2 ** (uint8(type(GovernorTypesV8.ProposalState).max) + 1)) - 1);

  /// @dev Thrown when the `proposalId` does not exist.
  /// @param proposalId The ID of the proposal that does not exist.
  error GovernorNonexistentProposal(uint256 proposalId);

  /// @dev Thrown when the current state of a proposal does not match the expected states.
  /// @param proposalId The ID of the proposal.
  /// @param current The current state of the proposal.
  /// @param expectedStates The expected states of the proposal as a bitmap.
  error GovernorUnexpectedProposalState(
    uint256 proposalId,
    GovernorTypesV8.ProposalState current,
    bytes32 expectedStates
  );

  /** ------------------ GETTERS ------------------ **/

  /**
   * @notice Retrieves the current state of a proposal.
   * @param self The storage reference for the GovernorStorage.
   * @param proposalId The ID of the proposal.
   * @return The current state of the proposal.
   */
  function state(
    GovernorStorageTypesV8.GovernorStorage storage self,
    uint256 proposalId
  ) external view returns (GovernorTypesV8.ProposalState) {
    return _state(self, proposalId);
  }

  /** ------------------ INTERNAL FUNCTIONS ------------------ **/

  /**
   * @dev Internal function to validate the current state of a proposal against expected states.
   * @param self The storage reference for the GovernorStorage.
   * @param proposalId The ID of the proposal.
   * @param allowedStates The bitmap of allowed states.
   * @return The current state of the proposal.
   */
  function validateStateBitmap(
    GovernorStorageTypesV8.GovernorStorage storage self,
    uint256 proposalId,
    bytes32 allowedStates
  ) internal view returns (GovernorTypesV8.ProposalState) {
    GovernorTypesV8.ProposalState currentState = _state(self, proposalId);
    if (encodeStateBitmap(currentState) & allowedStates == bytes32(0)) {
      revert GovernorUnexpectedProposalState(proposalId, currentState, allowedStates);
    }
    return currentState;
  }

  /**
   * @dev Encodes a `ProposalState` into a `bytes32` representation where each bit enabled corresponds to the underlying position in the `ProposalState` enum.
   * @param proposalState The state to encode.
   * @return The encoded state bitmap.
   */
  function encodeStateBitmap(GovernorTypesV8.ProposalState proposalState) internal pure returns (bytes32) {
    return bytes32(1 << uint8(proposalState));
  }

  /**
   * @notice Retrieves the current state of a proposal.
   * @dev See {IB3TRGovernor-state}.
   * @param self The storage reference for the GovernorStorage.
   * @param proposalId The ID of the proposal.
   * @return The current state of the proposal.
   */
  function _state(
    GovernorStorageTypesV8.GovernorStorage storage self,
    uint256 proposalId
  ) internal view returns (GovernorTypesV8.ProposalState) {
    // Load the proposal into memory
    GovernorTypesV8.ProposalCore storage proposal = self.proposals[proposalId];
    GovernorTypesV8.ProposalDevelopmentState proposalDevelopmentState = self.proposalDevelopmentState[proposalId];
    bool proposalExecuted = proposal.executed;
    bool proposalCanceled = proposal.canceled;

    if (proposalDevelopmentState == GovernorTypesV8.ProposalDevelopmentState.InDevelopment) {
      return GovernorTypesV8.ProposalState.InDevelopment;
    }

    if (proposalDevelopmentState == GovernorTypesV8.ProposalDevelopmentState.Completed) {
      return GovernorTypesV8.ProposalState.Completed;
    }

    if (proposalExecuted) {
      return GovernorTypesV8.ProposalState.Executed;
    }

    if (proposalCanceled) {
      return GovernorTypesV8.ProposalState.Canceled;
    }

    if (proposal.roundIdVoteStart == 0) {
      revert GovernorNonexistentProposal(proposalId);
    }

    // Check if the proposal is pending
    if (self.xAllocationVoting.currentRoundId() < proposal.roundIdVoteStart) {
      return GovernorTypesV8.ProposalState.Pending;
    }

    uint256 currentTimepoint = GovernorClockLogicV8.clock(self);
    uint256 deadline = GovernorProposalLogicV8._proposalDeadline(self, proposalId);

    if (!GovernorDepositLogicV8.proposalDepositReached(self, proposalId)) {
      return GovernorTypesV8.ProposalState.DepositNotMet;
    }

    if (deadline >= currentTimepoint) {
      return GovernorTypesV8.ProposalState.Active;
    } else if (
      !GovernorQuorumLogicV8.quorumReached(self, proposalId) || !GovernorVotesLogicV8.voteSucceeded(self, proposalId)
    ) {
      return GovernorTypesV8.ProposalState.Defeated;
    } else if (GovernorProposalLogicV8.proposalEta(self, proposalId) == 0) {
      return GovernorTypesV8.ProposalState.Succeeded;
    } else {
      bytes32 queueid = self.timelockIds[proposalId];
      if (self.timelock.isOperationPending(queueid)) {
        return GovernorTypesV8.ProposalState.Queued;
      } else if (self.timelock.isOperationDone(queueid)) {
        return GovernorTypesV8.ProposalState.Executed;
      } else {
        return GovernorTypesV8.ProposalState.Canceled;
      }
    }
  }
}
