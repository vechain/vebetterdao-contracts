...
library GovernorVotesLogicV8 {
  ...
  function _countVote(
    ...
    GovernorTypesV8.ProposalVote storage proposalVote = self.proposalVotes[proposalId];
    ...
    if (support == uint8(GovernorTypesV8.VoteType.Against)) {
      proposalVote.againstVotes += vote;
    } else if (support == uint8(GovernorTypesV8.VoteType.For)) {
      proposalVote.forVotes += vote;
    } else if (support == uint8(GovernorTypesV8.VoteType.Abstain)) {
   ...
   */
  function userVotedOnce(GovernorStorageTypesV8.GovernorStorage storage self, address user) internal view returns (bool) {
  ...
  function castVote(
    ...
    GovernorStateLogicV8.validateStateBitmap(
      self,
      proposalId,
      GovernorStateLogicV8.encodeStateBitmap(GovernorTypesV8.ProposalState.Active)
    );

    uint256 proposalSnapshot = GovernorProposalLogicV8._proposalSnapshot(self, proposalId);
    ...
    GovernorTypesV8.ProposalType proposalType = GovernorProposalLogicV8.proposalType(self, proposalId);
  ...
  function _checkVotingThreshold(
    ...
    uint256 threshold = GovernorConfiguratorV8.getVotingThreshold(self, proposalType);
    if (weight < threshold) {
   ...
   */
  function toggleQuadraticVoting(GovernorStorageTypesV8.GovernorStorage storage self) external {
    bool isQuadraticDisabled = self.quadraticVotingDisabled.upperLookupRecent(GovernorClockLogicV8.clock(self)) == 1; // 0: enabled, 1: disabled
    ...
    self.quadraticVotingDisabled.push(GovernorClockLogicV8.clock(self), newStatus);
