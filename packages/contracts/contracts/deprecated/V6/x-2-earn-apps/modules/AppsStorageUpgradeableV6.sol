...
import { AppStorageUtilsV6 } from "../libraries/AppStorageUtilsV6.sol";

/**
 * @title AppsStorageUpgradeableV6
 * @dev Contract to manage the x2earn apps storage.
 * Through this contract, the x2earn apps can be added, retrieved and indexed.
 */
abstract contract AppsStorageUpgradeableV6 is Initializable, X2EarnAppsUpgradeableV6 {
  ...
  function getPaginatedApps(uint startIndex, uint count) external view returns (X2EarnAppsDataTypes.App[] memory) {
    ...
    return AppStorageUtilsV6.getPaginatedApps($._apps, $._appIds, startIndex, count);
  }
