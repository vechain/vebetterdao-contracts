...
library GovernorVotesLogicV7 {
  ...
  function _countVote(
    ...
    GovernorTypesV7.ProposalVote storage proposalVote = self.proposalVotes[proposalId];
    ...
    if (support == uint8(GovernorTypesV7.VoteType.Against)) {
      proposalVote.againstVotes += vote;
    } else if (support == uint8(GovernorTypesV7.VoteType.For)) {
      proposalVote.forVotes += vote;
    } else if (support == uint8(GovernorTypesV7.VoteType.Abstain)) {
  ...
  function castVote(
    ...
    GovernorStateLogicV7.validateStateBitmap(
      self,
      proposalId,
      GovernorStateLogicV7.encodeStateBitmap(GovernorTypesV7.ProposalState.Active)
    );

    uint256 proposalSnapshot = GovernorProposalLogicV7._proposalSnapshot(self, proposalId);
    ...
    GovernorTypesV7.ProposalType proposalType = GovernorProposalLogicV7.proposalType(self, proposalId);
  ...
  function _checkVotingThreshold(
    ...
    uint256 threshold = GovernorConfiguratorV7.getVotingThreshold(self, proposalType);
    if (weight < threshold) {
   ...
   */
  function toggleQuadraticVoting(GovernorStorageTypesV7.GovernorStorage storage self) external {
    bool isQuadraticDisabled = self.quadraticVotingDisabled.upperLookupRecent(GovernorClockLogicV7.clock(self)) == 1; // 0: enabled, 1: disabled
    ...
    self.quadraticVotingDisabled.push(GovernorClockLogicV7.clock(self), newStatus);
