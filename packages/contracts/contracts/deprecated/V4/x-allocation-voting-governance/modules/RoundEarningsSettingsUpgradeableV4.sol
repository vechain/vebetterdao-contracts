...
/**
 * @title RoundEarningsSettingsUpgradeableV4
 * @notice Extension of {XAllocationVotingGovernorV4} to handle the settings for the x-allocation earnings calculations:
 ...
 */
abstract contract RoundEarningsSettingsUpgradeableV4 is Initializable, XAllocationVotingGovernorV4 {
  /// @custom:storage-location erc7201:b3tr.storage.XAllocationVotingGovernorV4.RoundEarningsSettings
  ...
  // keccak256(abi.encode(uint256(keccak256("b3tr.storage.XAllocationVotingGovernorV4.RoundEarningsSettings")) - 1)) & ~bytes32(uint256(0xff))
  ...
  function _setBaseAllocationPercentage(uint256 baseAllocationPercentage_) internal {
    require(
      ...
      "XAllocationVotingGovernorV4: Base allocation percentage must be less than or equal to 100"
    );
    EarningsSettingsStorage storage $ = _getEarningsSettingsStorage();
  ...
  function _setAppSharesCap(uint256 appSharesCap_) internal {
    require(appSharesCap_ <= 100, "XAllocationVotingGovernorV4: App shares cap must be less than or equal to 100");
    EarningsSettingsStorage storage $ = _getEarningsSettingsStorage();
