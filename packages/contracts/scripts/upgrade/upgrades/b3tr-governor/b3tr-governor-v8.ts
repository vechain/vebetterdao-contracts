...
async function main() {
  ...
  const governor = (await upgradeProxy("B3TRGovernorV7", "B3TRGovernor", config.b3trGovernorAddress, [], {
    version: 8,
    libraries: {
      GovernorClockLogic: await GovernorClockLogicLib.getAddress(),
      GovernorConfigurator: await GovernorConfiguratorLib.getAddress(),
  ...
  //Alert about the role
  console.log(
    "IMPORTANT: The B3TRGovernor contract has been upgraded to version 8. Please proceed with the following steps:",
  )
  console.log(
    "1. Grant the PROPOSAL_STATE_MANAGER_ROLE to a wallet that will be used to mark proposals as in development/completed",
  )
  console.log("2. Upgrade GrantsManager contract to version 2")
